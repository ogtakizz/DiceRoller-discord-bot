...
### 2. Criar e Configurar o Token do Discord
 - Para o bot funcionar, você precisa registrar uma aplicação no Discord:
    ...
    - **3.** No menu lateral, clique em **"Bot"**:
      - Clique em **"Reset Token"** para gerar sua chave e copie-a
      - Role até a seção **"Privileged Gateway Intents"** e **ATIVE** a opção **"Message Content Intent"** (caso contrário, o bot não irá ler as mensagens).
    - **4.** Na raiz do projeto, crie um arquivo chamado **`.env`**.
    - **5.** Adicione seu token no arquivo, seguindo este modelo:
      ```bash
